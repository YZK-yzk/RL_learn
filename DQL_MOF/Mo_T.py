...
print(type(env.action_space),type(env.observation_space))
...
a_indices = tf.stack([tf.range(BATCH_SIZE, dtype=tf.int32), tf_a], axis=1)
q_wrt_a = tf.gather_nd(params=q, indices=a_indices)              # shape=(None, ), q for current state #只需取当前动作的分数
